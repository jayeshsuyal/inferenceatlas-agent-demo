...
class CtoHandoffDocsTests(unittest.TestCase):
    ...
    def test_readme_top_fold_frames_public_harness(self) -> None:
        ...
        self.assertEqual(readme.splitlines()[0], "# InferenceAtlas")
        self.assertLessEqual(len(readme.splitlines()), 155)
        self.assertIn("Private engine, public proof.", first_screen)
        self.assertIn("docs/assets/proofgraph_banner.png", first_screen)
        self.assertTrue((ROOT / "docs" / "assets" / "proofgraph_banner.png").is_file())
        self.assertEqual(readme.count("!["), 1)
        self.assertIn("python3 -m web", first_screen)
        self.assertIn("scripts/review_run_rehearsal_gate.py", first_screen)
        self.assertNotIn("bash scripts/run.sh", first_screen)
        self.assertIn("not a private v1 code dump", readme)
        self.assertIn("This public harness does not approve access.", readme)
        self.assertIn("Downstream systems do not trust raw agent intent", first_screen)
        self.assertIn("Who Uses It", first_minute)
        self.assertIn("How IA Works", first_minute)
        self.assertIn("flowchart LR", first_minute)
        self.assertIn("Sponsor Proof", first_minute)
        self.assertIn("Tavily", first_minute)
        self.assertIn("Composio", first_minute)
        self.assertIn("OpenClaw", first_minute)
        self.assertIn("Nebius", first_minute)
        self.assertIn("IA Packet", first_minute)
        self.assertIn("Verification", readme)
        self.assertIn("Portkey guardrail", readme)
        self.assertIn("classDef hero", readme)
        self.assertIn("classDef sponsor", readme)
        self.assertIn("classDef downstream", readme)
        ...
        self.assertIn("zero writes", first_screen)
        self.assertIn("public contract v0", first_screen)
        self.assertIn("sponsor proof wired", first_screen)
        self.assertIn("Portkey guardrail path", first_screen)
        self.assertEqual(manifest["default_run_command"], "bash scripts/run.sh")
        self.assertEqual(manifest["readme_headline"], "InferenceAtlas")
        self.assertEqual(
