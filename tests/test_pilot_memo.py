...
class PilotMemoTests(unittest.TestCase):
    ...
    def test_copy_review_brief_is_short_safe_and_clipboard_ready(self) -> None:
        ...
        self.assertIn("Tavily sources evidence candidates", brief)
        self.assertIn("Composio drafts dry-run permission diffs", brief)
        self.assertIn("Nebius prepares reviewer narration", brief)
        self.assertIn("OpenClaw records runtime traces", brief)
        self.assertNotIn("finds discovery", brief)
        self.assertNotIn("simulates simulation", brief)
        self.assertNotIn("narrates narration", brief)
