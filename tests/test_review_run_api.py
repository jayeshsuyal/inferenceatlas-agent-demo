    ...
    def test_review_run_approval_receipt_api_is_read_only_and_packet_bound(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            with patch("web.app.REVIEW_RUN_STORE_DIR", store_dir):
                ...
                self.assertIn('class="receipt-console"', body)
                self.assertIn('class="object-header"', body)
                self.assertIn("ReviewRun / Receipt /", body)
                self.assertIn('class="fact-strip"', body)
                self.assertIn('class="audit-table"', body)
                self.assertIn('class="utility-rail"', body)
