...
class SkillsRegistryTests(unittest.TestCase):
    ...
    def test_skills_report_is_agent_readable(self) -> None:
        ...
        self.assertEqual(report["summary"]["registered_skills"], 17)
        self.assertEqual(report["summary"]["stable_skills"], 17)
        self.assertEqual(report["summary"]["available_stable_skills"], 17)
    ...
    def test_skills_cli_outputs_human_and_json_views(self) -> None:
        ...
        self.assertIn("17 / 17 stable skills available", text_result.stdout)
        self.assertIn("Artifact Integrity Verification", text_result.stdout)
        self.assertIn("Sponsor Proof Trace", text_result.stdout)
        self.assertEqual(json_result.returncode, 0, msg=json_result.stderr)
        payload = json.loads(json_result.stdout)
        self.assertEqual(payload["summary"]["available_stable_skills"], 17)
