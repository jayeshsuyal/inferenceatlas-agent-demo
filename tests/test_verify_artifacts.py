...
class VerifyArtifactsTests(unittest.TestCase):
    def test_checked_in_generated_artifacts_are_fresh(self) -> None:
        ...
        self.assertEqual(report["summary"]["generated_artifacts_verified"], 63)
    ...
    def test_verifier_reports_missing_actual_directory_without_crashing(self) -> None:
        ...
        self.assertEqual(report["status"], "fail")
        self.assertEqual(report["summary"]["stale_artifacts"], 63)
    ...
    def test_verify_artifacts_cli_text_is_skim_ready(self) -> None:
        ...
        self.assertIn("Evidence Receipt Ledgers (6 files): OK", result.stdout)
        self.assertIn("Packet Authority Snapshots (3 files): OK", result.stdout)
        self.assertIn("Packet Verification (3 files): OK", result.stdout)
        ...
        self.assertIn("Pilot Memo (3 files): OK", result.stdout)
        self.assertIn("ProofGraph Visual (1 files): OK", result.stdout)
        self.assertIn("Sponsor Evidence Replay (2 files): OK", result.stdout)
        self.assertIn("Sponsor Value Receipts (2 files): OK", result.stdout)
        self.assertIn("Sponsor Proof Trace (2 files): OK", result.stdout)
        self.assertIn("AI Spend Review (6 files): OK", result.stdout)
        self.assertIn("Total: 63 generated artifacts verified", result.stdout)
