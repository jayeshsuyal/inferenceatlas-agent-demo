...
class WebFilesTests(unittest.TestCase):
    ...
    def test_chat_empty_state_surfaces_locked_proof_state(self) -> None:
        ...
        self.assertIn("<strong>Sponsor proof:</strong> collect proof without changing the decision lock.", html)
        self.assertIn("<strong>Portkey:</strong> show the downstream verdict and no-mutation state.", html)
        self.assertIn("<strong>Export:</strong> copy the review brief or gate artifact.", html)
        ...
        self.assertIn("Collect proof while the decision lock stays unchanged.", html)
        self.assertIn("Show the downstream verdict, packet reference, and no-mutation state.", html)
        self.assertIn("Copy the review brief or export the gate artifact.", html)
        ...
        self.assertIn('["2 · IA Packet", "Inspect verdict, proof debt, owners, and hash."]', js)
        self.assertIn('["3 · Sponsor Proof", "Collect proof while the decision lock stays unchanged."]', js)
        self.assertIn('["4 · Portkey Proof Loop", "Show the downstream verdict, packet reference, and no-mutation state."]', js)
        self.assertIn('["5 · Export", "Copy the review brief or export the gate artifact."]', js)
        ...
        self.assertIn("packetDecisionLabel", js)
        self.assertIn("packet-status-card", js)
        self.assertIn("packet-meta-list", js)
        self.assertIn("sponsor-proof-strip", js)
        self.assertIn('return value ? "ready" : "blocked";', js)
        self.assertIn("Sponsors contribute evidence only. IA keeps the packet decision locked.", js)
        self.assertIn("Explain this packet", html)
        self.assertIn("optional sidecar", html)
        ...
        self.assertIn('body[data-active-tab="packet"] .stack', css)
        self.assertIn('body[data-active-tab="packet"] #btn-reset', css)
        ...
        self.assertIn(".packet-inline-coach-summary", css)
        self.assertIn(".packet-inline-coach-body", css)
        self.assertIn(".packet-status-card", css)
        self.assertIn(".status-pill", css)
        self.assertIn(".status-fact-grid", css)
        self.assertIn(".packet-meta-list", css)
        self.assertIn(".sponsor-proof-strip", css)
    ...
    def test_design_partner_walkthrough_ui_is_reachable(self) -> None:
        ...
        self.assertIn('data-tab="start"', html)
        self.assertIn("<span>Start Here</span>", html)
        self.assertIn("<small>90-second path</small>", html)
        ...
        self.assertIn("<span>Packet Review</span>", html)
        self.assertIn("<span>Proof Run</span>", html)
        self.assertIn("Sponsor Proof Run", html)
        self.assertIn('id="walkthrough-view"', html)
        self.assertIn("Private engine, public proof.", html)
        self.assertIn("/static/app.js?v=45", html)
